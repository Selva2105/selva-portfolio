'use client';

import { useState, useEffect } from 'react';
import Image from 'next/image';
import { DocIcon, LinkedinIcon, ExternalIcon } from '../icons';
import { LINKS } from '../../lib/links';
import heroDark from '../../public/hero-dark-image.png';
import heroLight from '../../public/hero-light-image.png';

// GitHub icon SVG component
function GithubIcon(props) {
  return (
    <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor" {...props}>
      <path fillRule="evenodd" clipRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.53 1.032 1.53 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" />
    </svg>
  );
}

export default function ContactProfileCard({
  imageDark = heroDark,
  imageLight = heroLight,
  showPhoto = true,
}) {
  const [istTime, setIstTime] = useState('');

  useEffect(() => {
    function updateTime() {
      try {
        const now = new Date();
        const formatter = new Intl.DateTimeFormat('en-US', {
          timeZone: 'Asia/Kolkata',
          hour: '2-digit',
          minute: '2-digit',
          second: '2-digit',
          hour12: true,
        });
        setIstTime(formatter.format(now));
      } catch (e) {
        setIstTime('');
      }
    }
    updateTime();
    const interval = setInterval(updateTime, 1000);
    return () => clearInterval(interval);
  }, []);

  return (
    <aside className="contact-profile-card">
      {showPhoto && (
        <div className="contact-photo-wrapper">
          <div className="contact-photo-box">
            <Image
              src={imageDark}
              alt="Selvaganapathi Kanakaraj"
              fill
              placeholder="blur"
              sizes="(max-width: 640px) 100vw, (max-width: 1024px) 340px, 380px"
              priority
              className="contact-photo-img contact-photo-dark"
            />
            <Image
              src={imageLight}
              alt=""
              aria-hidden="true"
              fill
              placeholder="blur"
              sizes="(max-width: 640px) 100vw, (max-width: 1024px) 340px, 380px"
              priority
              className="contact-photo-img contact-photo-light"
            />
            <span className="contact-photo-vignette" aria-hidden="true" />
          </div>

          <div className="contact-status-pill">
            <span className="pulse-dot" />
            <span>Available for new roles</span>
          </div>
        </div>
      )}

      <div className="contact-profile-body">
        <div className="contact-profile-head">
          <h2 className="contact-profile-name">Selvaganapathi Kanakaraj</h2>
          <p className="contact-profile-title">Full Stack / Frontend Developer</p>
        </div>

        <div className="contact-meta-row">
          <div className="contact-meta-item">
            <span className="meta-label">Location</span>
            <span className="meta-value">Bengaluru, India</span>
          </div>
          <div className="contact-meta-item">
            <span className="meta-label">Timezone</span>
            <span className="meta-value">
              IST (UTC+5:30)
              {istTime && <span className="contact-live-time">· {istTime}</span>}
            </span>
          </div>
          <div className="contact-meta-item">
            <span className="meta-label">Response Time</span>
            <span className="meta-value">Usually &lt; 24 hours</span>
          </div>
        </div>

        <div className="contact-profile-actions">
          <a
            href={LINKS.resume}
            download
            className="btn btn-p contact-resume-btn"
            title="Download Selvaganapathi's Résumé"
          >
            <DocIcon />
            <span>Download Résumé</span>
            <span className="resume-badge">PDF</span>
          </a>

          <div className="contact-social-grid">
            <a
              href={LINKS.linkedin}
              target="_blank"
              rel="noopener noreferrer"
              className="contact-social-link"
              aria-label="LinkedIn profile"
            >
              <LinkedinIcon />
              <span>LinkedIn</span>
              <ExternalIcon />
            </a>
            <a
              href={LINKS.github}
              target="_blank"
              rel="noopener noreferrer"
              className="contact-social-link"
              aria-label="GitHub profile"
            >
              <GithubIcon />
              <span>GitHub</span>
              <ExternalIcon />
            </a>
          </div>
        </div>
      </div>
    </aside>
  );
}
